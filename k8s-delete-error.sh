...
kubectl -n "$ns" get pods | grep -E "NotReady|Unknown|InvalidImageName|CrashLoopBackOff|Evicted|OOMKilled|Error|ImagePullBackOff" | awk '{print $1}' | xargs --no-run-if-empty kubectl -n "$ns" delete pod;
done
